...
class Type extends CoreModel
{
    ...
    public function getFooter()
    {
        ...
        $sql = 'select * from type where footer_order >= 1 order by footer_order';
        $pdoStatement = $pdo->query($sql);
        // 3. transformer le résultat en Objet
        $listObject = $pdoStatement->fetchAll(PDO::FETCH_CLASS, 'Type');
    ...
    public function find($id)
    {
        ...
        $pdoStatement = $pdo->query($sql);
        // 3. transformer le résultat en Objet
        $objetResultat = $pdoStatement->fetchObject('Type');
        // 4. renvoyer (return) le résultat
        return $objetResultat;
    }

    // cette méthode nous permet d'obtenir TOUS les types
